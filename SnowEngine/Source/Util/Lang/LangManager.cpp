...
void LangManager::set_lang(const String& lang)
{
	...
	{
		LOG_I(LANG_LOG_, L"Attempt to change the language to " + lang + L", which is already active");
		return;
	}
	...
	LOG_I(LANG_LOG_, L"The language has been changed to " + lang);
	if (errors > 0)
	{
		LOG_E(LANG_LOG_, L"Couldn't load some localization tables (" + util::to_string(errors) + L") while changing the language");
	}
}
...
void LangManager::load_table(const String& table, bool reload)
{
	...
	{
		LOG_I(LANG_LOG_, L"Attempt to load the localization table \"" + table + L"\" for " + current_lang_ + L", which has already been loaded");
		return;
	}

	load_table_(table, current_lang_, CURRENT_CONFIG.lang_path);
	LOG_I(LANG_LOG_, L"The localization table \"" + table + L"\" for " + current_lang_ + L" has been loaded");
}

bool LangManager::unload_table(const String& table)
{
	return table != CURRENT_CONFIG.lang_default_table && static_cast<bool>(strings_.erase(table));
}

int LangManager::unload_all_tables()
{
	int count = strings_.size() - 1;
	Table_ def_table = std::move(strings_.at(CURRENT_CONFIG.lang_default_table));
	strings_.clear();
	strings_.insert({ CURRENT_CONFIG.lang_default_table, std::move(def_table) });
	return count;
}
...
String LangManager::get_string(const String& table, const String& id, bool allow_loading)
{
	...
	auto table_iter = strings_.find(table);
	if (table_iter == strings_.end())
	{
		return table + L'.' + id;
	}

	auto str_iter = table_iter->second.find(id);
...
bool LangManager::is_valid(const String& table, const String& id, bool allow_loading)
{
	...
	return table_iter->second.find(id) != table_iter->second.end();
}
...
std::pair<String, String> LangManager::key_to_table_and_id(const String& key)
{
	if (!key.empty() && key != L'.')
	{
		std::size_t dot_pos = key.find(L'.');
		if (dot_pos == key.npos || dot_pos >= key.size() - 1) // No dot or it's the last character
		{
			return { CURRENT_CONFIG.lang_default_table, key };
		}
		else if (dot_pos == 0) // Dot is the first character
		{
			return { CURRENT_CONFIG.lang_default_table, key.substr(1, key.size() - 2) };
		}
		else // Dot is in the middle of the string
		{
			return { key.substr(0, dot_pos), key.substr(dot_pos + 1, key.size() - dot_pos - 2) };
		}
	}
	else
	{
...
void LangManager::load_table_(const String& table, const String& lang, const Path& path)
{
	std::shared_ptr<json::JsonObject> json = std::dynamic_pointer_cast<json::JsonObject>(
		json::Element::load(path / lang / (table + L".json")));
	if (!json)
	{
	...
	{
		t.insert({ k, util::json_to_string(v) });
	}
	strings_.erase(table);
	strings_.insert({ table, std::move(t) });
}
...
int LangManager::reload_(const Path& path)
{
	...
	for (const String& i : tables)
	{
		if (i != CURRENT_CONFIG.lang_default_table)
		{
			...
			{
				error_counter++;
				LOG_D(LANG_LOG_, L"Couldn't load the localization table \"" + i + L"\" for " + current_lang_);
			}
		}
	}
...
void LangManager::change_path_(const Config& new_config)
{
	...
	LOG_I(LANG_LOG_, L"The path of the localization directory has been changed");
	if (errors > 0)
	{
		LOG_E(LANG_LOG_, L"Couldn't load some localization tables (" + util::to_string(errors) + L") while changing the path");
	}
}
